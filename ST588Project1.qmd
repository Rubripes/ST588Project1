...
retrieveCensus <- function(year=2022,numericVars=c("AGEP","PWGTP"),catVars=c("SEX"),geographyLevel =c("All"),geographySpec = NULL){
 ...
 ## If all variables are of valid length and valid values, paste them together to be passed in the API call
  ...
  ##This doesn't seem to return an object in the Global Environment, added object output
  censusData <- return(as_tibble(censusData))
}
```
...
```{r}
state17 <- retrieveCensus(geographyLevel = "State", geographySpec = 17)
```
...
```{r}
censusData<-retrieveCensus(year=2022, numericVars=c("AGEP", "PWGTP", "JWAP", "JWDP"),    catVars = c("SEX", "HHL"))
```
...
## make tibbles from dataset variables by data type and remove null values
...
dataCatVars
dataNumVars
rm(dataCatVars, dataNumVars, catVarsTblShort)
```
