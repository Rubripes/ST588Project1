...
varsLkpTibble <- function(year,varName,lkpVal){
...
lkpToMiddleTime <- function(tibble,year=2022,timeVar){
  ...
  returnData[[timeVar]] <- returnData$MidTime
  #data <- data %>%
...
```{r}
state17 <- retrieveCensus(year=2022,numericVars=c("AGEP","PWGTP","JWDP"),geographyLevel = "State",geographySpec = 17)
...
splMean <- list()
for(i in 1:length(numCensus)){
  splMean[[i]] <- as.numeric((sum(numCensus[[i]] %*% censusData$PWGTP)/sum(censusData$PWGTP)))
  names(splMean)[i]<-paste("mean", names(numCensus[i]), sep="_")
}
...
splSD <- list()
for(i in 1:length(numCensus)){
  splSD[[i]] <- (sqrt(sum(numCensus[[i]]^2 %*% censusData$PWGTP)/sum(censusData$PWGTP)-splMean[[i]]^2))
  names(splSD)[i]<-paste("SD", names(numCensus[i]), sep="_")
}

#Create loop to generate counts for each categorical variable
splCount <- list()
for(i in 1:length(catCensus)){
  splCount[[i]]<- catCensus[i] |>
    group_by(catCensus[i]) |>
    summarize(n=n())
}
