...
```{r}
censusStateSummary <- censusSummerizR(x = censusStateData, numericVars = c("AGEP"), catVars = c("SEX","SCHL"))
...
```{r}
if (!is.vector(censusStateData[["AGEP"]]) || !is.vector(censusStateData$PWGTP)) {
  stop("Both arguments must be vectors.")
}

# Ensure both vectors are row vectors or column vectors
if (length(dim(censusStateData[["AGEP"]])) == 2 && length(dim(censusStateData$PWGTP)) == 2) {
  # If both are matrices, check if they have the same dimensions
  if (dim(censusStateData[["AGEP"]]) != dim(censusStateData$PWGTP)) {
    stop("Matrix dimensions are not compatible.")
  }
} else {
  # If one or both are vectors, ensure they have the same length
  if (length(censusStateData[["AGEP"]]) != length(censusStateData$PWGTP)) {
    stop("Vector lengths are not compatible.")
  }
}
