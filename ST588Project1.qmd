...
```{r}
retrieveCensus <- function(year=2022,numericVars=c("AGEP","PWGTP"),catVars=c("SEX"),geographyLevel =c("All")){
  ...
  #if (!all(numericVars %in% numericOptions)) {
 #   stop("Invalid variables: ", paste(numericVars[!numericVars %in% numericOptions], collapse = #", "))
#  }
  
  catOptions <- c("FER","HHL","HISPEED","JWTRNS","SCH","SCHL","SEX")
  
  if (!all(catVars %in% catOptions)) {
    stop("Invalid variables: ", paste(catVars[!catVars %in% catOptions], collapse = ", "))
  }
  
  geographyOptions <- c("All","Region","Division","State")
  
  if (!all(geographyLevel %in% geographyOptions)) {
    stop("Invalid variables: ", paste(geographyLevel[!geographyLevel %in% geographyOptions], collapse = ", "))
  }
