...
_ If the user passes another value in geography level and does NOT include a geography spec, return everything for that level in the format (example) of "state:*", with an asterik representing all.
...
#check user requested variables and create numeric and factor/character datasets to summarize seperately based on user choice
...
#Summaries to Output from Function
return(c(numericSumNamed,catSummary))
...
```{r}
censusSummerizR(censusData, numericVars = c("AGEP", "GASP"), catVars = c( "SEX"))
```

```{r}
#censusSummerizR <- function(x,
...
numericDataDefault <- censusData |>
  select(where(is.numeric) & -"PWGTP")

charDataDefault <- censusData|>
  select(where(is.factor))


length(names(numericDataDefault))
       
if(length(names(numericDataDefault))<length(numericVars)){
  stop("You cannot subset data that was not pulled from API, re-pull desired variables")
}

if(length(charDataDefault)<length(catVars)){
  stop("You cannot subset data that was not pulled from API, re-pull desired variables")
}


if(length(numericDataDefault$names)>length(numericVars)){
  numericData<- x |> 
    select(numericVars)
  } else {
  numericData<- numericDataDefault 
...
if(length(charDataDefault$names)>length(catVars)){
  charData<- x |> 
    select(catVars)
  } else {
  charData<- charDataDefault 
