...
retrieveCensus <- function(year=2022,numericVars=c("AGEP","PWGTP"),catVars=c("SEX"),geographyLevel =c("All"),geographySpec = NULL){
 ...
 ## If all variables are of valid length and valid values, paste them together to be passed in the API call
  ...
  #lkpToMiddleTime(data,year=2022,timeVar = "JWDP")
...
```{r}
state17 <- retrieveCensus(numericVars = c("AGEP","PWGTP","JWAP","JWDP"),catVars = c("FER","SCHL","SEX"),geographyLevel = "State", geographySpec = 17)
...
test <- lkpToMiddleTime(state17,timeVar = "JWDP")
test
#names(test)[1:(length(names(test))-1)]
